...
void ModelRenderer::Render(const RenderContext& rc, const DirectX::XMFLOAT4X4& worldTransform, const Model* model, ShaderId shaderId, const DirectX::XMFLOAT4& color)
{
	...
	// update color CB
	CbMesh cbColor{};
	cbColor.materialColor = color;
	dc->UpdateSubresource(meshConstantBuffer.Get(), 0, 0, &cbColor, 0, 0);
	...
	for (const ModelResource::Mesh& mesh : resource->GetMeshes())
	{
		...
		for (const ModelResource::Subset & subset : mesh.subsets)
		{
			...
			// ✅ Force OUR CbMesh to PS b1 (slot 1)
			ID3D11Buffer* b1 = meshConstantBuffer.Get();
			dc->PSSetConstantBuffers(1, 1, &b1);

			// ✅ Overwrite materialColor LAST so it wins
			CbMesh cbMesh{};
			cbMesh.materialColor = color;
			dc->UpdateSubresource(meshConstantBuffer.Get(), 0, nullptr, &cbMesh, 0, 0);
